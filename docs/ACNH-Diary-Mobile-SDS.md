...
acnh-diary-mobile/
  src/
    ...
    db/
      database.ts
    domain/       # planned
    data/         # bundled reference data, asset maps, and assets
      content/encyclopedia/{bugs,fish,sea,fossils,art}.json
      content/catalog/catalog.json
      content/catalog/catalog-variations.json
      encyclopedia.ts
      encyclopedia-labels.ts
      encyclopedia-assets.ts
      encyclopedia-detail-assets.ts
      catalog.ts
      catalog-assets.ts
      music-assets.ts
    types/
...
type CollectionItemType = ItemType | CatalogCategory;
...
- 카탈로그 수집 단위는 `(islandId, catalogType, itemId)` 기본 아이템이다. 변형 목록은 상세 화면에서 보여 주지만 MVP에서는 변형별 상태를 저장하지 않는다.
...
주민 목록은 417명 번들 데이터를 메모리에서 검색·필터·정렬한다. 카드를 선택하면 `/villagers/[villagerId]` 독립 상세 화면으로 이동하며, 상세 화면은 공통 `AppTopBar`의 뒤로가기·햄버거 메뉴와 독립 스크롤을 제공한다. 상세에는 아이콘·전체·포스터·액자·하우스 외관·하우스 내부 이미지를 제공하고, 기준 데이터에 없는 활동 시간·가구 값은 행을 숨긴다. 액자 사진 보유 토글은 주민 상태의 `photo_received`와 연결하며, 포스터 보유는 `poster_owned`로 별도 저장한다. 캠핑장 방문일은 `YYYY-MM-DD`로 검증한 뒤 `campsite_visits`에 중복 없이 저장한다.
...
도감 홈과 카탈로그 홈은 `CollectionHomeShell`을 사용해 같은 시각 계층을 유지한다. 셸은 앱 전체에 적용되는 `#FFFCF5` 아이보리 웜 화이트 캔버스만 제공하며 배경 이미지, 외곽 라운드 패널, 장식 원을 렌더링하지 않는다. `CollectionHomeSummaryCard`는 수집률·수집 수·진행바를 공통 렌더링하며, `CollectionHomeCategoryCard`는 아이콘·분류명·세부 지표·진행바를 2열로 표시한다. 도감 카드는 카테고리별 채집·기증 또는 보유·기증 또는 진품·가품·기증 지표를 주입하고, 카탈로그 카드는 보유 지표를 주입한다. 모든 화면의 `AppTopBar`는 동일한 높이·좌우 여백·`#FFFDFC` 헤더 배경·하단 경계를 사용한다.

`AppBottomNav`는 5개 탭의 공통 하단 내비게이션 컴포넌트다. 각 탭의 동일한 터치 영역·간격·안전 영역·라벨 스타일을 유지하고 모든 탭에 `tabBarActive: #A7D18C`, `tabBarActiveSurface: #F0F8EA`, `tabBarActiveText: #356B43`, `tabBarInactive: #849087`를 동일하게 적용한다. 본문 배경은 `#FFFCF5`, 헤더는 `#FFFDFC`, 하단 바는 `#FFFFFF`를 사용한다. 주요 액션은 `#79AC5F`, 보조 표면과 플로팅 버튼은 `#DCEFD2`, 본문 강조 텍스트는 `#3F4A43`, 경계선은 `#BCD9A8`으로 관리하고, 카테고리·상태 보조색은 핑크·블루·연노랑·연보라 계열의 밝은 색상으로 구성한다. `useTabBarVisibility`가 숨김을 요청하면 같은 컴포넌트가 레이아웃에서 사라지고, 복원 시 동일한 바 스타일로 돌아온다.
...
다음 두 항목은 구현 방향에 영향을 주므로 사용자 결정이 필요하다.
...
추가로 기준 데이터 자동 업데이트 방식, 백업 파일 암호화 여부, 오늘·주민 상세에 남은 물음표 필드의 최종 표시 여부는 해당 기능 구현 직전에 결정해도 된다.
...
- v2.4 · 2026-08-30 · 카탈로그 상세정보 기반 분류 태그·크기·기능·리폼·행운·주문 가능 여부 필터 반영
- v2.5 · 2026-08-30 · 주민 상세 바텀시트를 독립 route 화면으로 전환하고 목록·상세 내비게이션 계약을 정합화
