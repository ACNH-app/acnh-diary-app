...
- 주민 상태·성격·종류 분류 selector와 공통 상단 검색
- 도감 필터·정렬·표시 단위
- 루틴 유효기간과 완료 상태
- 컬렉션 상태 허용 필드 검증
...
5개 탭은 `AppBottomNav`를 공통 사용한다. 탭의 구조·간격·안전 영역·표시/숨김 동작·활성 색상은 모두 공유하며, 화이트 바탕 위에 연두색 활성 상태·진한 연두색 활성 텍스트와 중립 회색 비활성 상태를 사용한다. 주민·도감·공략 화면은 `AppChrome`과 `AppTopBar`의 접이식 검색을 공통 사용한다. 주민 목록의 상단바에는 밑줄 없는 상태·성격·종류 텍스트 selector를 배치하고, 본문 `UnderlineTabs`의 데이터만 선택한 분류에 따라 교체한다.
